...
export const handler = async (event /*, context*/) => {
  ...
  if (table) {
    ...
    models.fields.forEach((field) => {
      ...
      // definitions?.[camelTable]? is an array of objects with a name property
      field.definition = definitions?.[camelTable]?.find((item) => {
        return item.name === field.name
      })
    })
  }
  // only include the fields from the definition
  models.fields = definitions?.[camelTable]?.map((item) => {
    return models.fields.find((field) => {
