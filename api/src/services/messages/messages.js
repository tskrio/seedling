...
export const messages = async ({ filter, skip, orderBy, q, take }) => {
  try {
    let _take = (() => {
      let limit =
        take ||
        parseInt(context.currentUser.preferences['message.pageSize'], 10) ||
...
export const message = async ({ cuid }) => {
  try {
...
export const updateMessage = async ({ cuid, input }) => {
  try {
...
export const deleteMessage = async ({ cuid }) => {
  try {
